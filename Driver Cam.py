...
label.pack(side=TOP)
filename = PhotoImage(file=r"C:\Users\ravin\Desktop\Drowsiness\demo.png")
...
def webdet():
   capture =cv2.VideoCapture(0)
   face_cascade = cv2.CascadeClassifier(r'C:\Users\ravin\Desktop\Drowsiness\lbpcascade_frontalface.xml')
   eye_glass = cv2.CascadeClassifier(r'C:\Users\ravin\Desktop\Drowsiness\haarcascade_eye_tree_eyeglasses.xml')
...
def webdetRec():
   ...
   eye_glass = cv2.CascadeClassifier(r'C:\Users\ravin\Desktop\Drowsiness\haarcascade_eye_tree_eyeglasses.xml')
   fourcc=cv2.VideoWriter_fourcc(*'XVID') 
   op=cv2.VideoWriter(r'C:\Users\ravin\Desktop\Drowsiness\NewSample2.avi',fourcc,9.0,(640,480))
...
def alert():
   path=os.path.join(r"C:\Users\ravin\Desktop\Drowsiness","beep-07.wav")
   mixer.init()
   alert=mixer.Sound(path)
   alert.play()
   time.sleep(0.1)
   alert.play()   
...
def blink():
   capture =cv2.VideoCapture(0)
   face_cascade = cv2.CascadeClassifier(r'C:\Users\ravin\Desktop\Drowsiness\lbpcascade_frontalface.xml')
   eye_cascade = cv2.CascadeClassifier(r'C:\Users\ravin\Desktop\Drowsiness\haarcascade_eye.xml')
   blink_cascade = cv2.CascadeClassifier(r'C:\Users\ravin\Desktop\Drowsiness\CustomBlinkCascade.xml')
...
but6=Button(frame,padx=5,pady=5,width=39,bg='white',fg='black',relief=GROOVE,command=blink,text='customize alerm sound',font=('helvetica 15 bold'))
but6.place(x=5,y=472)

but7=Button(frame,padx=5,pady=5,width=5,bg='white',fg='black',relief=GROOVE,text='EXIT',command=exitt,font=('helvetica 15 bold'))
but7.place(x=210,y=478)
